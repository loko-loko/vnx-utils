...
select_mode_bay(){ #NO ARG
	...
	if [[ $_OPT_MODE =~ Info|Remove ]]; then
        local BAY_LIST_ARRAY=($(awk '$0 ~ /CKM/ {print $1}' /sansto/etc/vnx/vnx_exec_list.txt))
        
    elif [[ $_OPT_MODE == 'Create' ]]; then
        BAY_LIST_ARRAY=($(awk '$0 ~ /CKM/ && $2 == "Pool" {print $1}' /sansto/etc/vnx/vnx_exec_list.txt))
	
    elif [[ $_OPT_MODE == 'Modify' ]]; then
        BAY_LIST_ARRAY=($(awk '$0 ~ /CKM/ && $3 == "E" {print $1}' /sansto/etc/vnx/vnx_exec_list.txt))
